...
export function useNaverTourismMarkers({
  ...
  useEffect(() => {
    ...
    for (const place of visiblePlaces) {
      ...
      visibleSignature += (visibleSignature.length > 0 ? '|' : '') + `${place.id}:${place.latitude}:${place.longitude}`;
    }
    ...
    // Performance Optimization: Prevent O(N) array allocations and GC pressure
    // by using direct mutations and standard `for...of` loops rather than
    // declarative chains (.map, .filter, spreads) during marker reconciliation.
