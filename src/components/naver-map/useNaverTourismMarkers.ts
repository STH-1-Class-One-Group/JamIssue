...
export function useNaverTourismMarkers({
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Combine multiple .map() and .join() passes into a single loop to avoid allocating intermediate arrays.
    ...
    // ⚡ Bolt: Prevent O(N) array allocations by using direct push within loops instead of .map(), .filter(), and spread syntax.
