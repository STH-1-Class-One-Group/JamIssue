...
export function useNaverTourismMarkers({
  ...
  useEffect(() => {
    ...
    const idsToRefreshForOps = new Set<string>();
    if (previousSelectedTourismPlaceIdRef.current) {
      idsToRefreshForOps.add(previousSelectedTourismPlaceIdRef.current);
    }
    if (selectedTourismPlaceId) {
      idsToRefreshForOps.add(selectedTourismPlaceId);
    }

    for (const placeId of idsToRefreshForOps) {
      operations.push(() => {
        const place = placeById.get(placeId);
        const marker = tourismMarkersRef.current.get(placeId);
        if (place && marker) {
