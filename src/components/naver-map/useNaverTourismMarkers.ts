...
export function useNaverTourismMarkers({
  ...
  useEffect(() => {
    ...
    // Bolt: Combined loop to avoid intermediate allocations from .map().join() and new Map()
    ...
    // Bolt: Use simple for-loops instead of .filter().map(), ...spread to avoid large intermediate allocations
