# TSK-014-04 KTO user-value information sheet fix
...
| Gate | Evidence |
| --- | --- |
| Responsibility map | `tourismClient`는 Worker API path만 소유하고, coordinator effect는 상세 lazy-load와 cache만 소유한다. `TourismInfoSheet`는 list/detail DTO를 user-facing view로 렌더링한다. |
...
| Test seam | API client path tests, sheet rendering unit tests, Playwright API fixture detail route로 회귀를 고정했다. |
...
- PR 생성 후 PR URL, merge SHA, CI URL을 이 문서와 `#434`에 기록한다.
- Main merge 후 `#425` parent issue에 최종 증거를 누적한다.
