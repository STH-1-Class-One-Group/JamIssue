...
| TSK-012-20 | https://github.com/STH-1-Class-One-Group/JamIssue/issues/482 | https://github.com/STH-1-Class-One-Group/JamIssue/pull/483 | 4f953b36427aa262528be493d78d1b1abe7b0165 | docs/TASK_ID_LEDGER.md | KTO ON/분류 전환 시 Naver HTML marker 삽입이 입력 응답성을 막는 문제를 marker cap, RAF batch scheduling, diff 기반 SDK mutation 축소로 해결했다. | 운영 API는 정상 응답하지만 프론트가 KTO marker를 한 번에 과도하게 materialize하면 screenshot timeout과 renderer freeze가 재발할 수 있었다. | 모바일 cap 32, 데스크톱 cap 64, fallback cap 48, batch size 8, requestAnimationFrame scheduler, remove/create/selected-refresh diff로 변경했다. | API payload 경량화, edge cache, server-side viewport query는 backend/admin 후속 후보다. | completed |
| TSK-012-21 | https://github.com/STH-1-Class-One-Group/JamIssue/issues/485 | https://github.com/STH-1-Class-One-Group/JamIssue/pull/486 | a7e9a9b161dccd4106566d8a8bcd325d9bec19c6 | docs/traceability/task-ledger.jsonl; docs/TASK_ID_LEDGER.md; docs/ISSUE_TREE.md | 백엔드가 KTO `scope=all`을 KV snapshot compact read model로 변경했으므로 Web Front 소비 계약을 새 fast path에 맞추기 위해 해결했다. | list response의 `total`이 optional이었고, `sourceReady=false`/HTTP 503 상태가 재시도 가능한 KTO layer 비활성 상태로 고정되지 않았다. | `TourismPlacesResponse.total`을 필수화하고, KTO ON은 `scope=all` 1회 호출, 필터는 local filtering, detail은 `/api/tourism/places/{id}` lazy-load로 테스트를 보강했다. | Naver SDK client id 없는 E2E 환경에서는 marker click detail sheet를 직접 검증하지 못해 detail lazy-load는 hook seam에서 검증했다. | completed |
