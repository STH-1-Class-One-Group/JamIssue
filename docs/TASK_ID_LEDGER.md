...
| TSK-012-18 | https://github.com/STH-1-Class-One-Group/JamIssue/issues/474 | https://github.com/STH-1-Class-One-Group/JamIssue/pull/477 | 4d57037a77afb184320ba06887eb971d33d85044 | docs/traceability/task-ledger.jsonl; docs/TASK_ID_LEDGER.md; docs/ISSUE_TREE.md | 지도 플로팅 캡슐 클릭 시 renderer freeze처럼 보이는 회귀와 Naver 기본 컨트롤 침범을 방지하기 위해 처리했다. | `전체` 필터 클릭은 API 호출 없이 즉시 열려야 하는데 운영에서 screenshot timeout 또는 freeze처럼 관찰됐고, Naver 기본 줌 컨트롤이 캡슐 레이어를 침범했다. | PR #477에서 드롭다운 400ms 표시, screenshot 1초 성공, 관광정보 API 호출 부재를 E2E로 고정하고 Naver 기본 줌 컨트롤을 map adapter 경계에서 비활성화했다. | KTO 최초 전체 로딩 성능은 별도 initial load hardening 범위로 남긴다. | completed |
